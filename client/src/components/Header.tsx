    ...
    <header className="bg-white shadow-lg fixed top-0 left-0 right-0 z-50 backdrop-blur-sm">
      ...
      <div className="container mx-auto py-4 px-4 lg:px-0">
        <div className="flex flex-col md:flex-row justify-between items-center">
          ...
          {/* REPLACE: Move Search (fixed rectangle) left of Apply Now */}
          <div className="flex items-center justify-between mt-4 md:mt-0 gap-4">
            {/* Fixed-size Search button (rectangle with label) */}
            <div className="flex items-center gap-3">
              <button
                onClick={toggleSearchModal}
                className="px-4 py-2 bg-white border border-gray-200 rounded-lg text-primary font-medium hover:shadow-sm transition"
            ...
            {/* Hamburger */}
            <button
              className="block md:hidden text-primary focus:outline-none p-2 rounded-full hover:bg-gray-100 transition-colors"
      ...
      {isSearchOpen && (
        ...
        >
          <div
            className="mt-24 w-full max-w-md bg-white rounded-xl shadow-2xl border border-gray-200 overflow-hidden"
            role="dialog"
            aria-modal="true"
          >
            ...
            <div className="flex items-center justify-between px-4 py-3 border-b border-gray-100">
              ...
              <button
                onClick={toggleSearchModal}
                className="text-gray-500 hover:text-gray-800 focus:outline-none p-2 rounded-full"
            ...
            <div className="px-4 py-3">
              <div className="relative">
                <input
                  ...
                  className="w-full pl-3 pr-3 py-2 text-base border border-gray-200 rounded-md focus:outline-none focus:ring-2 focus:ring-primary"
                  value={searchTerm}
            ...
            {/* Results area — NO popular suggestions or defaults.
                Shows only when user has typed something. */}
                ...
                <div className="space-y-2">
                  {filteredResults.map((item, idx) => (
                    <button
                      key={idx}
                      onClick={() => {
                        handleResultClick(item.link);
                        setSearchTerm("");
                        setSearchOpen(false);
                      }}
                      className="w-full text-left p-3 bg-white rounded-md border border-gray-100 hover:bg-primary/5 transition"
                    >
